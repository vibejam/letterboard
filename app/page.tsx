...
export default function Home() {
  ...
  const [board, setBoard] = useState<BoardViewData>(defaultBoardViewData);
  useEffect(() => {
    let active = true;
  ...
  return <main className="site-shell">
    ...
    <div className="live-strip"><span className="live-dot" /><strong>Live board</strong><span>·</span><span>{board.stats.claimed} places claimed</span><span>·</span><span>{board.stats.total - board.stats.claimed} places open</span></div>
    ...
    <section className="board-intro-strip"><div><strong>Founding 100</strong><span>Free public profiles for the first newsletters shaping the board.</span></div><div><strong>{board.stats.claimed} / {board.stats.total}</strong><span>places claimed</span></div><div><strong>Next phase</strong><span>Spotlight placement after 100 confirmed profiles.</span></div></section>
