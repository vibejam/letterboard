...
export function ClaimFlow({ open, onClose, initialNewsletter, liveNewsletter, onClaimCreated, boardStatsOverride }: ClaimFlowProps) {
  ...
  useEffect(() => {
    if (liveNewsletter?.status !== "confirmed") return;
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section className="claim-modal" role="dialog" aria-modal="true" aria-labelledby="claim-title">
    ...
    {step === "success" && <div className="claim-step success-step"><div className="success-hero"><span className="success-check">✓</span><p className="hero-label">{status === "confirmed" ? "FOUNDING STATUS CONFIRMED" : "PENDING CONFIRMATION"}</p><h3>{status === "confirmed" ? "Your Founding Mark is live." : "Your place is reserved."}</h3><p>{status === "confirmed" ? "You are confirmed on Letterboard and your public profile is ready to share." : "Your pending profile is reserved. Confirm ownership from the email to activate your Founding Mark."}</p><Boardmark status={status} tier={foundingTier} size="large" /></div><div className="success-position"><span className="success-position__rank">#{String(sharePosition).padStart(2, "0")}</span><div><strong>{newsletter.name}</strong><span>Founding place / 100</span></div><span className="success-position__status">{status === "confirmed" ? "ACTIVE" : "PENDING"}</span></div><div className="success-actions">{shareProps ? <ShareProfileButton {...shareProps} /> : null}<button className="secondary-button" onClick={() => setStep("profile")}>Preview public profile <span>→</span></button></div><p className="optional-note">Sharing is optional. It helps the board become visible; it does not gate your place.</p></div>}
    {step === "profile" && <div className="claim-step profile-step"><div className="profile-preview-header"><div><p className="hero-label">PUBLIC PROFILE</p><h3>{newsletter.name}</h3><p>{newsletter.url}</p></div><Boardmark status={status} tier={foundingTier} /></div><div className="profile-preview-card"><div className="profile-preview-card__main"><NewsletterLogo src={newsletter.logoUrl} alt={`${newsletter.name} logo`} initials={newsletter.initials} tone={newsletter.tone} /><div><span className="profile-preview-card__category">{newsletter.category}</span><h4>{newsletter.name}</h4><p>{newsletter.description}</p></div></div><div className="profile-preview-card__meta"><span><strong>#{String(sharePosition).padStart(2, "0")}</strong><small>Founding place</small></span><span><strong>{status === "confirmed" ? "Ready" : "Pending"}</strong><small>Founding Mark</small></span></div></div><div className="profile-share-preview"><div><p className="hero-label">SHARE CARD</p><h4>Make your place visible.</h4><p>Use the same profile details and Founding Mark everywhere you share it.</p></div><ShareCard name={newsletter.name} url={newsletter.url} category={newsletter.category} description={newsletter.description} rank={sharePosition} status={status} tier={foundingTier} initials={newsletter.initials} tone={newsletter.tone} logoUrl={newsletter.logoUrl} share={shareProps ?? undefined} /></div><div className="success-actions">{shareProps ? <ShareProfileButton {...shareProps} label="Share your place →" /> : null}<button className="secondary-button" onClick={onClose}>Done</button></div></div>}
  </section></div>;
}
