...
export function ActivityPanel({ events }: { events: BoardActivity[] }) {
  return <section className="activity-grid" aria-label="Live board activity">
    <div className="activity-panel"><div className="panel-kicker"><span className="kicker-star">✦</span> Trending in the inbox</div><div className="activity-list">{events.slice(0, 4).map((event) => <div className="activity-row" key={`${event.name}-${event.time}`}><Avatar initials={event.name.slice(0, 1)} tone={event.tone} /><strong>{event.name}</strong><span>{event.detail}</span></div>)}</div></div>
    <div className="activity-panel"><div className="panel-kicker"><span className="live-dot" /> Latest activity</div><div className="activity-list">{events.map((event) => <div className="activity-row" key={`${event.name}-${event.time}`}><Avatar initials={event.name.slice(0, 1)} tone={event.tone} /><strong>{event.name}</strong><span>{event.time}</span></div>)}</div></div>
  </section>;
}
