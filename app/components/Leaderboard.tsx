...
function TopCard({ item, rank, onClaim }: { item: Newsletter; rank: number; onClaim: (item?: Newsletter) => void }) {
  ...
  return <article className={`top-card ${rank === 1 ? "top-card--first" : ""}`}>
    <div className="top-card__rank">#{String(rank).padStart(2, "0")}</div><NewsletterLogo src={item.logoUrl} alt={`${item.name} logo`} initials={item.initials} tone={item.tone} />
    ...
    <div className="top-card__actions">{externalUrl && item.status === "confirmed" ? <a className="text-button" href={externalUrl} target="_blank" rel="noopener noreferrer">Read newsletter <span aria-hidden="true">↗</span></a> : item.slug ? <a className="text-button" href={`/${item.slug}`}>View public profile <span aria-hidden="true">→</span></a> : <button className="text-button" onClick={() => onClaim(item)}>View profile <span aria-hidden="true">→</span></button>}{item.slug && <a className="text-button top-card__profile-link" href={`/${item.slug}`}>View profile <span aria-hidden="true">→</span></a>}</div>
  </article>;
}
...
function BoardRow({ item, rank, onClaim }: { item: Newsletter; rank: number; onClaim: (item?: Newsletter) => void }) {
  ...
  if (item.status === "confirmed") {
    return <a className="board-row" href={item.slug ? `/${item.slug}` : safeExternalUrl(item.url) ?? "#board"}>{contents}</a>;
  }
...
export function Leaderboard({ onClaim, data }: { onClaim: (item?: Newsletter) => void; data: BoardViewData }) {
  ...
  return <section className="board-section" id="board" aria-labelledby="board-title">
    ...
    {data.leaderboard.length > 0 ? <><div className="top-stack" aria-label="Top three founding newsletters"><div className="top-stack__label">TOP 3</div>{first && <TopCard item={first} rank={1} onClaim={onClaim} />}{second && <TopCard item={second} rank={2} onClaim={onClaim} />}{third && <TopCard item={third} rank={3} onClaim={onClaim} />}</div><div className="board-table" aria-label="Founding 100 newsletter leaderboard"><div className="table-heading"><span>FOUNDING PLACE</span><span>NEWSLETTER</span><span>TOPIC</span><span>VIEWS</span><span>STATUS</span></div>{data.leaderboard.slice(3).map((item, index) => <BoardRow item={item} rank={index + 4} onClaim={onClaim} key={item.id} />)}</div></> : <div className="empty-board" role="status"><Boardmark status="pending" size="large" /><h3>Who gets there first?</h3><p>Letterboard starts empty. The first verified newsletter takes #01.</p><button className="primary-button" onClick={() => onClaim()}>Claim #01 <span>→</span></button></div>}
