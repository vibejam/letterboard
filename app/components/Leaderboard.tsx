...
function TopCard({ item, rank, onClaim }: { item: Newsletter; rank: number; onClaim: (item?: Newsletter) => void }) {
  ...
  const profilePath = confirmed && item.slug && safeSlug.test(item.slug) ? `/${encodeURIComponent(item.slug)}` : null;
  return <article className={`top-card ${rank === 1 ? "top-card--first" : ""} ${confirmed ? "top-card--clickable" : ""}`} tabIndex={confirmed ? 0 : undefined} role={confirmed ? "link" : undefined} onClick={(event) => openCard(event, item, destination)} onKeyDown={(event) => openCard(event, item, destination)}>
    ...
    <div className="top-card__body"><div className="top-card__heading"><div>{confirmed ? <h3><ExternalLink item={item} destination={destination}>{item.name}</ExternalLink></h3> : <h3>{item.name}</h3>}<p>{item.category}</p></div><strong className="top-card__signal">{confirmed ? (rank === 1 ? "LIVE ON THE BOARD" : "FOUNDING PLACE") : "PENDING REVIEW"}</strong></div><p className="top-card__description">{item.description}</p><div className="top-card__meta"><span>{item.url}</span><span>{formatClicks(item.clicks)}</span><Boardmark status={item.status} tier={item.foundingTier ?? tierForRank(rank)} size="small" /></div></div>
...
function BoardRow({ item, rank, onClaim }: { item: Newsletter; rank: number; onClaim: (item?: Newsletter) => void }) {
  const contents = <>
    ...
    <span className="board-row__status"><Boardmark status={item.status} tier={item.foundingTier ?? tierForRank(rank)} size="small" /><span>{item.status === "confirmed" ? "Confirmed Founding Mark" : "Pending review"}</span><span aria-hidden="true">{item.status === "confirmed" ? "→" : ""}</span></span>
  </>;
...
export function Leaderboard({ onClaim, data }: { onClaim: (item?: Newsletter) => void; data: BoardViewData }) {
  ...
  const { stats } = data;
  const nextPosition = data.leaderboard.length ? Math.max(...data.leaderboard.map((item) => item.foundingPosition ?? 0), 0) + 1 : 1;
  return <section className="board-section" id="board" aria-labelledby="board-title">
    <div className="section-intro section-intro--compact"><div><p className="eyebrow">THE LIVE BOARD</p><h2 id="board-title">{data.leaderboard.length ? "The live board." : "Who gets there first?"}</h2></div><p className="section-note">{data.leaderboard.length ? `The next founding place is waiting at #${String(nextPosition).padStart(2, "0")}.` : "Letterboard starts empty. The first verified newsletter takes #01."}</p></div>
    {data.leaderboard.length > 0 ? <><div className="top-stack" aria-label="Top three founding newsletters"><div className="top-stack__label">TOP 3</div>{first && <TopCard item={first} rank={first.foundingPosition ?? 1} onClaim={onClaim} />}{second && <TopCard item={second} rank={second.foundingPosition ?? 2} onClaim={onClaim} />}{third && <TopCard item={third} rank={third.foundingPosition ?? 3} onClaim={onClaim} />}</div><div className="board-table" aria-label="Founding 100 newsletter leaderboard"><div className="table-heading"><span>FOUNDING PLACE</span><span>NEWSLETTER</span><span>TOPIC</span><span>CLICKS</span><span>STATUS</span></div>{data.leaderboard.slice(3).map((item, index) => <BoardRow item={item} rank={item.foundingPosition ?? index + 4} onClaim={onClaim} key={item.id} />)}</div></> : <div className="empty-board" role="status"><Boardmark status="pending" size="large" /><h3>Who gets there first?</h3><p>Letterboard starts empty. The first verified newsletter takes #01.</p><button className="primary-button" onClick={() => onClaim()}>Claim #01 <span>→</span></button></div>}
    <div className="board-footer-line"><span>{stats.reserved} reserved · {stats.claimed} claimed · {stats.open} open</span><button className="text-button" onClick={() => onClaim()}>Claim my spot <span aria-hidden="true">→</span></button></div>
  </section>;
}
