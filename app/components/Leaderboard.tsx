...
function TopCard({ item, rank, onClaim }: { item: Newsletter; rank: number; onClaim: (item?: Newsletter) => void }) {
  return <article className={`top-card ${rank === 1 ? "top-card--first" : ""}`}>
    ...
    <div className="top-card__body"><div className="top-card__heading"><div><h3>{item.name}</h3><p>{item.category}</p></div><strong className="top-card__signal">{rank === 1 ? "FIRST ON THE BOARD" : "FOUNDING PLACE"}</strong></div><p className="top-card__description">{item.description}</p><div className="top-card__meta"><span>{item.url}</span><span>{formatNumber(item.clicks)} profile views</span><Boardmark tier={tierForRank(rank)} size="small" /></div></div>
