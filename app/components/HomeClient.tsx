...
export default function HomeClient({ initialBoard }: { initialBoard: BoardViewData }) {
  ...
  return <main className="site-shell">
    ...
    <div className="live-strip"><span className="live-dot" /><strong>FOUNDING 100</strong><span>·</span><span>{board.stats.reserved} {board.stats.reserved === 1 ? "place" : "places"} reserved</span><span>·</span><span>{board.stats.claimed} {board.stats.claimed === 1 ? "place" : "places"} claimed</span><span>·</span><span>{board.stats.open} {board.stats.open === 1 ? "place" : "places"} open</span></div>
    <section className="hero" id="top"><div className="hero__content"><p className="hero-label">THE FOUNDING 100 IS OPEN</p><h1>Be one of the first 100 newsletters on Letterboard.</h1><p className="hero__lede">Claim your permanent founding position, build your public profile, and be there before the leaderboard opens to the world.</p><p className="hero__trust">Free forever to claim. No card. No catch.</p><button className="hero-claim-bar" onClick={() => openClaim()}><span>Paste your newsletter URL</span><strong>Claim my spot <span>→</span></strong></button><p className="hero__helper">Your place is reserved after email and publication ownership verification.</p><p className="hero__tiers">#1–5 OG · #6–10 Legend · #11–50 Icon · #51–100 Pioneer · The first 100 keep their Founding Mark permanently.</p></div></section>
    <Leaderboard onClaim={openClaim} data={board} />
    <ActivityPanel events={board.activity} />
    <section className="board-intro-strip"><div><strong>Founding 100</strong><span>The first 100 verified newsletters become permanent founding members of Letterboard.</span></div><div><strong>{board.stats.reserved} reserved · {board.stats.claimed} claimed</strong><span>{board.stats.open} places open</span></div><div><strong>NEXT → SPOTLIGHT</strong><span>After the Founding 100 closes, Spotlight opens for featured visibility without changing organic founding status.</span></div></section>
    <section className="how-section" id="how-it-works"><div><p className="eyebrow">HOW THE BOARD WORKS</p><h2>Simple in. Visible out.</h2></div><div className="how-grid"><article><span>01</span><h3>Paste your newsletter</h3><p>Drop in your URL. Letterboard builds the public profile.</p></article><article><span>02</span><h3>Prove it’s yours</h3><p>Confirm your email, then prove you control the publication before your Founding Mark activates.</p></article><article><span>03</span><h3>Get on the board</h3><p>Your newsletter goes live, gets discovered and starts building its place in the ranking.</p></article></div></section>
    ...
    <ClaimFlow key={`${claimOpen}-${selectedNewsletter?.id ?? "new"}`} open={claimOpen} onClose={() => { setClaimOpen(false); setSelectedNewsletter(undefined); setClaimTarget(undefined); }} onClaimCreated={setClaimTarget} initialNewsletter={selectedNewsletter} liveNewsletter={liveNewsletter} />
  </main>;
}
