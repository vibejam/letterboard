...
export async function POST(request: Request) {
  ...
  if (!claimId || !creatorEmail) return NextResponse.json({ error: "INVALID_RESEND" }, { status: 400 });
  ...
  if (result.error || !result.data) return NextResponse.json({ error: "INVALID_RESEND" }, { status: 400 });
  if (result.data.status !== "pending" || result.data.contact_email !== creatorEmail) return NextResponse.json({ error: "INVALID_RESEND" }, { status: 400 });
  const newsletter = result.data.newsletters as unknown as { title: string } | null;
  if (!newsletter?.title) return NextResponse.json({ error: "INVALID_RESEND" }, { status: 400 });
