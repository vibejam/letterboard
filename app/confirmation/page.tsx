  ...
  return <main className="site-shell confirmation-page">
    ...
    <section className="confirmation-card" aria-live="polite">
      ...
      {confirmed ? <>
        ...
        <div className="confirmation-actions"><Link className="primary-button" href={`/${slug}`}>View my public profile <span>→</span></Link>{newsletterUrl ? <a className="secondary-button" href={newsletterUrl} target="_blank" rel="noopener noreferrer">Open newsletter <span>↗</span></a> : null}<ShareProfileButton slug={slug!} newsletterName={title} foundingPosition={Number(position)} tier={tier!} sourcePlatform={sourcePlatform} newsletterUrl={newsletterUrl} /><Link className="secondary-button" href="/">Return to the board</Link></div>
      </> : <>
