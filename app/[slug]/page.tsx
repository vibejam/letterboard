...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  return <main className="site-shell profile-page">
    ...
    <section className="public-profile-card"><p className="hero-label">PUBLIC PROFILE / CONFIRMED</p><div className="public-profile-card__heading"><LetterboardMark /><div><h1>{result.data.title}</h1><p>{result.data.canonical_url}</p></div></div><Boardmark status="confirmed" tier={tier} size="large" /><p className="public-profile-card__description">{result.data.description}</p><div className="public-profile-card__meta"><span><strong>#{String(result.data.founding_position).padStart(2, "0")}</strong><small>Founding place</small></span><span><strong>{String(result.data.founding_tier).toUpperCase()}</strong><small>Boardmark tier</small></span><span><strong>Confirmed</strong><small>Ownership status</small></span></div></section>
