...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  const result = await supabase.from("newsletters").select("id,slug,title,description,logo_url,logo_source,canonical_url,source_platform,founding_position,founding_tier,profile_views,ownership_status,boardmark_status").eq("slug", slug).eq("ownership_status", "confirmed").maybeSingle();
  ...
  const category = result.data.source_platform ?? "Newsletter";
  return <main className="site-shell profile-page">
    ...
    <section className="public-profile-card">
      ...
      <div className="public-profile-card__meta"><span><strong>#{String(result.data.founding_position).padStart(2, "0")}</strong><small>Founding place</small></span><span><strong>{String(result.data.founding_tier).toUpperCase()}</strong><small>Founding Mark tier</small></span><span><strong>{Number(result.data.profile_views ?? 0).toLocaleString("en-US")}</strong><small>Profile views</small></span></div>
      <div className="profile-primary-actions">{externalUrl ? <a className="primary-button" href={externalUrl} target="_blank" rel="noopener noreferrer">Read newsletter <span>↗</span></a> : <span className="form-note">The newsletter link is temporarily unavailable.</span>}<ShareProfileButton slug={slug} newsletterName={result.data.title} foundingPosition={result.data.founding_position ?? 0} tier={tier ?? "og"} sourcePlatform={result.data.source_platform} newsletterUrl={externalUrl} /></div>
    </section>
    <section className="profile-share-section"><div><p className="eyebrow">SHARE CARD</p><h2>Make the founding place visible.</h2><p>Share the confirmed profile and Founding Mark wherever people discover your newsletter.</p></div><ShareCard name={result.data.title} url={slug} category={category} description={result.data.description ?? "A public newsletter on Letterboard."} rank={result.data.founding_position ?? 0} status="confirmed" tier={tier} initials={initials} tone="paper" logoUrl={safeExternalUrl(result.data.logo_url) ?? undefined} share={{ slug, newsletterName: result.data.title, foundingPosition: result.data.founding_position ?? 0, tier: tier ?? "og", sourcePlatform: result.data.source_platform, newsletterUrl: externalUrl }} /></section>
