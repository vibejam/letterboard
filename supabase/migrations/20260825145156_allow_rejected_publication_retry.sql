-- Reopen only a historical, non-active publication inside the same transaction
-- that creates its next claim revision. The original claim and timestamps stay
-- intact; the newsletter row remains the single canonical publication record.
...
declare
  ...
  v_profile_slug text;
begin
  -- Serialize retries for one normalized publication, while the row lock below
  -- serializes with the existing retirement and confirmation transactions.
  ...
  if found then
    if exists (
      select 1 from public.claims
       where newsletter_id = v_existing.id
         and status in ('pending', 'confirmed')
    ) then
      raise exception 'PUBLICATION_ALREADY_CLAIMED';
    end if;

    -- A confirmed publication remains authoritative even if historical data
    -- is inconsistent and has no active claim row.
    ...
    -- Retiring/reopening does not overwrite the original confirmation or
    -- claimed timestamps, and it never deletes the prior claim or profile row.
    ...
    exception when unique_violation then
      -- A concurrent insert wins the publication race. The caller can retry
      -- and will then take the locked existing-row path above.
      raise exception 'PUBLICATION_ALREADY_CLAIMED';
    end;
  end if;
